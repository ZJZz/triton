...
它把 AssignLatencies 留下的 tt.latency 或已有 serialized schedule
转成 loop.stage / loop.cluster / tt.scheduled_max_stage，
把“这个 op 属于第几个 pipeline stage、粗粒度顺序如何”这个 contract
...
- Schedule data structure: [Schedule.h](/LocalRun/jiangzhe.zhao/my_repo/triton/include/triton/Dialect/TritonGPU/Transforms/Schedule.h:43)
...
| Arch | Before feature | After feature | Result |
| --- | --- | --- | --- |
...
| Blackwell `sm100` | `ttng.tc_gen5_mma` has `{tt.latency = 1, tt.self_latency = 1}` | `tt.latency` is removed; loop body ops get `{loop.stage = 0, loop.cluster = 0}`; loop gets `{tt.scheduled_max_stage = 0}` | changed |
...
#### 5.1 Ampere `sm86`: no visible change in canonical matmul
...
#### 5.2 Hopper `sm90`: no visible change in canonical matmul
...
#### 5.3 Blackwell `sm100`: consumes `tt.latency`, materializes schedule attrs
...
为什么这里 `scheduled_max_stage = 0`？见 §7.2 的公式。简化算式是：

```text
tc_gen5_mma 没有普通 in-loop user；computeDistance 跳过 scf.yield terminator。
...
所以 pass 仍然生效，但这不是一个多 stage overlap 的展示样本。

## Part C. Scheduling Mechanism

### 6. Source Mapping

#### 6.1 Top-level dispatch: `getInitialSchedule`
...
#### 6.2 Read and remove latency attrs
...
#### 6.3 Warp-specialize existing-schedule branch
...
#### 6.4 Add prologue/epilogue `scf.if`
...
#### 6.5 Schedule remaining ops and materialize attrs
...
Stage assignment 的主算法在 `scheduleKeyOps`，见 [ScheduleLoops.cpp:152](/LocalRun/jiangzhe.zhao/my_repo/triton/lib/Dialect/TritonGPU/Transforms/Pipeliner/ScheduleLoops.cpp:152)。
...
- `user` 必须能映射回当前 loop body 的 top-level op。
- `scf.yield` terminator 被跳过。
- 如果没有 in-loop user，`maxDist = -1`，被当成 0。
- 这个 distance 是 latency 加权最长路径，不是 def-use hop count。

直观上：

- 路径越长、延迟越大的 op 越应该早发起，因此 stage 越小。
- 越靠近消费端的 op 越靠后，因此 stage 越大。
- `tt.latency = 2` 比 `tt.latency = 1` 拉开更多 stage，是因为 latency 值作为路径权重直接累加。
...
#### 7.2 Why sm100 canonical matmul has only stage 0
...
所以 `scheduleKeyOps` 计算出的可见 coarse schedule 只有一个 stage。

#### 7.3 Is the result optimal?

不是。`ScheduleLoops` 产出的是一个可行的、偏启发式的 coarse schedule，不是对“总执行周期最小”之类目标的全局最优解。

原因：

- `scheduleKeyOps` 只基于 latency anchor 和 def-use 图计算 stage。
...
### 8. Data Model: `CoarseSchedule`, Stage, Cluster, Serialize

#### 8.1 `CoarseSchedule`

`CoarseSchedule` 是 `ScheduleLoops` / `Pipeline` 使用的核心中间调度数据结构，定义见 [Schedule.h:43](/LocalRun/jiangzhe.zhao/my_repo/triton/include/triton/Dialect/TritonGPU/Transforms/Schedule.h:43)。
...
```text
loop body 的 coarse pipeline schedule 表
```

为什么叫 coarse：

- 它不是 cycle-level 硬件调度。
- 它不表达 scoreboard、issue slot 或精确时钟。
- 它只表达“第几层 stage”和“粗粒度顺序组”。

主要用途：

- 作为 `ScheduleLoops` 在 pass 内部逐步构造和修补 schedule 的工作数据结构。
- 作为 `loop.stage` / `loop.cluster` / `tt.scheduled_max_stage` 的内存态语义表示。
- 作为后续 `Pipeline` / `LowerLoops` 线性化和展开 loop 的顺序依据。

#### 8.2 `stage`

`loop.stage` 不是精确执行时刻，而是跨 pipeline stage 的相对时间层级：

- `stage=0` 的 op 应该比 `stage=1` 更早被发起。
- `stage=1` 的 op 应该比 `stage=2` 更早。

但只靠 `loop.stage` 不足以决定全部顺序。同 stage 内仍然需要 `cluster` 约束。

#### 8.3 `cluster`

这里的 `cluster` 不是 GPU hardware cluster，也不是 CTA cluster。它是 `CoarseSchedule::ClusterList` 上的一条全局 ordering chain 里的粗粒度顺序组。

为什么需要它：

- 只知道两个 op 都在 `stage=0`，仍不足以决定顺序。
- 还需要知道谁靠前，哪些 op 应该作为一个顺序块被一起放到前面或后面。

`scheduleKeyOps` 的初始 cluster 构造见 [ScheduleLoops.cpp:212](/LocalRun/jiangzhe.zhao/my_repo/triton/lib/Dialect/TritonGPU/Transforms/Pipeliner/ScheduleLoops.cpp:212)：
...
这里的 `serialize` 不是写 JSON 或磁盘文件，而是：

```text
把 pass 内存里的 CoarseSchedule，
物化成 IR 上的 scheduling attributes。
```

对应实现见 [Schedule.cpp:255](/LocalRun/jiangzhe.zhao/my_repo/triton/lib/Dialect/TritonGPU/Transforms/Pipeliner/Schedule.cpp:255)：

- 对每个 scheduled op 写 `loop.stage` / `loop.cluster`
- 对 `scf.for` 写 `tt.scheduled_max_stage`

为什么必须这样做：

- `CoarseSchedule` 只是 pass 运行时的 C++ 对象。
- pass 结束后它会消失。
- 后续 `Pipeline` / `LowerLoops` 只能通过 IR attrs 读取这个调度结果。

所以：

```text
serialize:   CoarseSchedule -> IR attrs
deSerialize: IR attrs -> CoarseSchedule
```

### 9. Dependency Completion

#### 9.1 Normal SSA dependencies
...
```text
把 IR 里已经存在的 SSA def-use 依赖，
递归补进 CoarseSchedule。
```

真正递归回溯的是 [Schedule.cpp:52](/LocalRun/jiangzhe.zhao/my_repo/triton/lib/Dialect/TritonGPU/Transforms/Pipeliner/Schedule.cpp:52) 的 `insertDepsOfOp`：

- 枚举 `getNestedOperands(op)`，不仅看直接 operand，也看 nested region 内部 operand。
- 找每个 operand 的 defining op。
- 若 defining op 也在同一个 loop body 中，则把它插入当前 schedule。
- 继续递归处理这个 defining op 的 operands。

#### 9.2 Backward def-use closure
...
```text
backward def-use closure
= 从一个 op 出发，沿 use -> def 方向一直回溯，
  收集所有递归可达的上游 producer。
```
...
若当前 scheduled op 是 `%3`，那么它的 backward def-use closure 包括 `%2`、`%1`、`%0`。这和只看一层直接 dependency 不同。

#### 9.3 Multiple consumers sharing one producer

一个 producer 被多个 consumer 需要时，`scheduleDependencies` 不会复制 producer，而是通过 `insertMinimum` 取一个“足够早”的 placement，见 [Schedule.cpp:16](/LocalRun/jiangzhe.zhao/my_repo/triton/lib/Dialect/TritonGPU/Transforms/Pipeliner/Schedule.cpp:16)：

- 如果新的要求更早 stage，就更新为更早 stage。
- 如果 stage 相同但新的 cluster 更早，就更新为更早 cluster。
- 如果新的要求更晚，则保持原 placement。

规则是：

```text
producer 至少要早到能满足所有 consumer，
因此保留最早的那个 placement。
```

这里处理的是静态 dependency closure，不是运行时“并发执行”建模：

- 有 def-use，就保留顺序约束。
- 没有 def-use，`scheduleDependencies` 不会额外强加依赖。

#### 9.4 Loop-carried distance-1 dependencies
...
- `tt.load` 有例外：跟当前 op 放同 stage / cluster。
- 其他 op 放到下一 stage 且在当前 cluster 前，见 [ScheduleLoops.cpp:72](/LocalRun/jiangzhe.zhao/my_repo/triton/lib/Dialect/TritonGPU/Transforms/Pipeliner/ScheduleLoops.cpp:72)。

为什么这能保证后续 Pipeline 有足够 stage 信息：

```text
ScheduleLoops 不是支持任意复杂 loop。
它先通过 isSafeToPipeline 拒绝 distance > 1 等复杂情况，
再对支持的 normal SSA deps 和 loop-carried distance-1 deps 做完整补全。
```

因此更准确的边界是：

```text
ScheduleLoops 不保证 stage 个数全局最优，
但对它接受的 loop，它的目标是生成一个依赖自洽、足够后续 Pipeline 使用的 coarse schedule。
```
...
### 14. GPU / Hardware / Optimization Reason
...
## Part E. Reference And Follow-ups
...
ScheduleLoops
  -> reads and removes tt.latency
  -> builds CoarseSchedule: Operation* -> (stage, cluster)
  -> completes normal SSA deps and distance-1 loop-carried deps
  -> serializes schedule to IR attrs

Pipeline / LowerLoops
  -> consumes loop.stage / loop.cluster / tt.scheduled_max_stage
  -> rewrites loop into actual pipelined control flow
```

Key terms:

- `stage`: relative pipeline time layer, not exact cycle.
- `cluster`: global coarse ordering group, not GPU hardware cluster.
- `serialize`: materialize in-memory schedule as IR attrs.
- `closure`: recursively reachable full set, e.g. all upstream producers in backward def-use traversal.
...
- `loop.cluster` 的最终效果最好结合 `Pipeline` / `LowerLoops` 再读一次，因为它的完整语义体现在 loop 展开后的 linearized schedule。
