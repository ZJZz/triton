...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_kernel(%a_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32} loc("a_ptr"(#loc)), %b_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32} loc("b_ptr"(#loc)), %c_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32} loc("c_ptr"(#loc)), %M: i32 loc("M"(#loc)), %N: i32 loc("N"(#loc)), %K: i32 loc("K"(#loc)), %stride_am: i32 loc("stride_am"(#loc)), %stride_ak: i32 loc("stride_ak"(#loc)), %stride_bk: i32 loc("stride_bk"(#loc)), %stride_bn: i32 loc("stride_bn"(#loc)), %stride_cm: i32 loc("stride_cm"(#loc)), %stride_cn: i32 loc("stride_cn"(#loc))) attributes {noinline = false} {
    ...
    %acc_44 = ttg.memdesc_index %acc_36[%c0_i32] : !ttg.memdesc<2x1xi64, #shared, #smem, mutable> -> !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc62)
    ttng.tc_gen5_mma %a_41, %b_43, %acc, %false, %acc_39, %acc_44[%true] {is_async} : !ttg.memdesc<64x32xf16, #shared1, #smem>, !ttg.memdesc<32x64xf16, #shared2, #smem>, !ttg.memdesc<64x64xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc53)
    %a_ptrs_45 = tt.addptr %a_ptrs_20, %a_ptrs_33 : tensor<64x32x!tt.ptr<f16>, #blocked>, tensor<64x32xi32, #blocked> loc(#loc50)
    %b_ptrs_46 = tt.addptr %b_ptrs_31, %b_ptrs_35 : tensor<32x64x!tt.ptr<f16>, #blocked1>, tensor<32x64xi32, #blocked1> loc(#loc52)
    %acc_47 = arith.subi %K, %c32_i32 : i32 loc(#loc62)
    %acc_48:8 = scf.for %acc_61 = %c0_i32 to %acc_47 step %c32_i32 iter_args(%a_ptrs_62 = %a_ptrs_45, %b_ptrs_63 = %b_ptrs_46, %acc_64 = %c0_i32, %acc_65 = %c1_i32, %acc_66 = %acc_44, %acc_67 = %c0_i32, %a_68 = %a_41, %b_69 = %b_43) -> (tensor<64x32x!tt.ptr<f16>, #blocked>, tensor<32x64x!tt.ptr<f16>, #blocked1>, i32, i32, !ttg.memdesc<1xi64, #shared, #smem, mutable>, i32, !ttg.memdesc<64x32xf16, #shared1, #smem>, !ttg.memdesc<32x64xf16, #shared2, #smem>)  : i32 {
      ttng.wait_barrier %acc_66, %acc_67 deps %a_68, %b_69 : !ttg.memdesc<1xi64, #shared, #smem, mutable>, !ttg.memdesc<64x32xf16, #shared1, #smem>, !ttg.memdesc<32x64xf16, #shared2, #smem> loc(#loc53)
      %a_70 = tt.load %a_ptrs_62 : tensor<64x32x!tt.ptr<f16>, #blocked> loc(#loc55)
      %a_71 = ttg.local_alloc %a_70 : (tensor<64x32xf16, #blocked>) -> !ttg.memdesc<64x32xf16, #shared1, #smem> loc(#loc55)
      %b_72 = tt.load %b_ptrs_63 : tensor<32x64x!tt.ptr<f16>, #blocked1> loc(#loc56)
      %b_73 = ttg.local_alloc %b_72 : (tensor<32x64xf16, #blocked1>) -> !ttg.memdesc<32x64xf16, #shared2, #smem> loc(#loc56)
      %acc_74 = ttg.memdesc_index %acc_36[%acc_65] : !ttg.memdesc<2x1xi64, #shared, #smem, mutable> -> !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc62)
      ttng.tc_gen5_mma %a_71, %b_73, %acc, %true, %true, %acc_74[%true] {is_async} : !ttg.memdesc<64x32xf16, #shared1, #smem>, !ttg.memdesc<32x64xf16, #shared2, #smem>, !ttg.memdesc<64x64xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc53)
      %a_ptrs_75 = tt.addptr %a_ptrs_62, %a_ptrs_33 : tensor<64x32x!tt.ptr<f16>, #blocked>, tensor<64x32xi32, #blocked> loc(#loc50)
      %b_ptrs_76 = tt.addptr %b_ptrs_63, %b_ptrs_35 : tensor<32x64x!tt.ptr<f16>, #blocked1>, tensor<32x64xi32, #blocked1> loc(#loc52)
      %acc_77 = arith.xori %acc_64, %c1_i32 : i32 loc(#loc53)
      %acc_78 = arith.addi %acc_65, %c1_i32 : i32 loc(#loc53)
      %acc_79 = arith.cmpi sge, %acc_78, %c2_i32 : i32 loc(#loc53)
      %acc_80 = arith.select %acc_79, %c0_i32, %acc_78 : i32 loc(#loc53)
      %acc_81 = arith.select %acc_79, %acc_77, %acc_64 : i32 loc(#loc53)
      scf.yield %a_ptrs_75, %b_ptrs_76, %acc_81, %acc_80, %acc_74, %acc_64, %a_71, %b_73 : tensor<64x32x!tt.ptr<f16>, #blocked>, tensor<32x64x!tt.ptr<f16>, #blocked1>, i32, i32, !ttg.memdesc<1xi64, #shared, #smem, mutable>, i32, !ttg.memdesc<64x32xf16, #shared1, #smem>, !ttg.memdesc<32x64xf16, #shared2, #smem> loc(#loc62)
    } loc(#loc62)
    %acc_49 = arith.cmpi sgt, %K, %c0_i32 : i32 loc(#loc62)
    scf.if %acc_49 {
      ttng.wait_barrier %acc_48#4, %acc_48#5 deps %acc_48#6, %acc_48#7 : !ttg.memdesc<1xi64, #shared, #smem, mutable>, !ttg.memdesc<64x32xf16, #shared1, #smem>, !ttg.memdesc<32x64xf16, #shared2, #smem> loc(#loc53)
    } loc(#loc62)
    %acc_50 = ttg.memdesc_index %acc_36[%c0_i32] : !ttg.memdesc<2x1xi64, #shared, #smem, mutable> -> !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc62)
    ttng.inval_barrier %acc_50 : !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc62)
    %acc_51 = ttg.memdesc_index %acc_36[%c1_i32] : !ttg.memdesc<2x1xi64, #shared, #smem, mutable> -> !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc62)
    ttng.inval_barrier %acc_51 : !ttg.memdesc<1xi64, #shared, #smem, mutable> loc(#loc62)
    ttg.local_dealloc %acc_36 : !ttg.memdesc<2x1xi64, #shared, #smem, mutable> loc(#loc62)
    %acc_52 = ttng.tmem_load %acc : !ttg.memdesc<64x64xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<64x64xf32, #linear> loc(#loc53)
    %c = arith.truncf %acc_52 : tensor<64x64xf32, #linear> to tensor<64x64xf16, #linear> loc(#loc57)
    %c_ptrs = tt.splat %stride_cm : i32 -> tensor<64x1xi32, #blocked1> loc(#loc58)
    %c_ptrs_53 = arith.muli %c_ptrs, %a_ptrs_9 : tensor<64x1xi32, #blocked1> loc(#loc58)
    %c_ptrs_54 = tt.splat %c_ptr : !tt.ptr<f16> -> tensor<64x1x!tt.ptr<f16>, #blocked1> loc(#loc59)
    %c_ptrs_55 = tt.addptr %c_ptrs_54, %c_ptrs_53 : tensor<64x1x!tt.ptr<f16>, #blocked1>, tensor<64x1xi32, #blocked1> loc(#loc59)
    %c_ptrs_56 = tt.splat %stride_cn : i32 -> tensor<1x64xi32, #blocked1> loc(#loc60)
    %c_ptrs_57 = arith.muli %c_ptrs_56, %b_ptrs_26 : tensor<1x64xi32, #blocked1> loc(#loc60)
    %c_ptrs_58 = tt.broadcast %c_ptrs_55 : tensor<64x1x!tt.ptr<f16>, #blocked1> -> tensor<64x64x!tt.ptr<f16>, #blocked1> loc(#loc59)
    %c_ptrs_59 = tt.broadcast %c_ptrs_57 : tensor<1x64xi32, #blocked1> -> tensor<64x64xi32, #blocked1> loc(#loc59)
    %c_ptrs_60 = tt.addptr %c_ptrs_58, %c_ptrs_59 : tensor<64x64x!tt.ptr<f16>, #blocked1>, tensor<64x64xi32, #blocked1> loc(#loc59)
    %0 = ttg.convert_layout %c : tensor<64x64xf16, #linear> -> tensor<64x64xf16, #blocked1> loc(#loc25)
    tt.store %c_ptrs_60, %0 : tensor<64x64x!tt.ptr<f16>, #blocked1> loc(#loc25)
    tt.return loc(#loc)
  } loc(#loc)
} loc(#loc)
#loc1 = loc(unknown)
