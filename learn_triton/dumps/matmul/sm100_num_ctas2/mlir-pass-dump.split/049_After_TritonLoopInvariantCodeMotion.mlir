...
module attributes {"ttg.num-ctas" = 2 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_kernel(%a_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32} loc("a_ptr"(#loc)), %b_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32} loc("b_ptr"(#loc)), %c_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32} loc("c_ptr"(#loc)), %M: i32 loc("M"(#loc)), %N: i32 loc("N"(#loc)), %K: i32 loc("K"(#loc)), %stride_am: i32 loc("stride_am"(#loc)), %stride_ak: i32 loc("stride_ak"(#loc)), %stride_bk: i32 loc("stride_bk"(#loc)), %stride_bn: i32 loc("stride_bn"(#loc)), %stride_cm: i32 loc("stride_cm"(#loc)), %stride_cn: i32 loc("stride_cn"(#loc))) attributes {noinline = false} {
    ...
    %acc:3 = scf.for %k = %c0_i32 to %K step %c32_i32 iter_args(%b_ptrs_45 = %b_ptrs_31, %acc_46 = %cst, %a_ptrs_47 = %a_ptrs_20) -> (tensor<32x64x!tt.ptr<f16>, #blocked2>, tensor<64x64xf32, #linear>, tensor<64x32x!tt.ptr<f16>, #blocked>)  : i32 {
      ...
      %acc_50, %acc_51 = ttng.tmem_alloc %acc_46 : (tensor<64x64xf32, #linear>) -> (!ttg.memdesc<64x64xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token) loc(#loc58)
      %acc_52 = ttng.tc_gen5_mma %a_48, %b_49, %acc_50[%acc_51], %true, %true : !ttg.memdesc<64x32xf16, #shared, #smem>, !ttg.memdesc<32x64xf16, #shared1, #smem>, !ttg.memdesc<64x64xf32, #tmem, #ttng.tensor_memory, mutable> loc(#loc58)
      %acc_53, %acc_54 = ttng.tmem_load %acc_50[%acc_52] : !ttg.memdesc<64x64xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<64x64xf32, #linear> loc(#loc58)
      %a_ptrs_55 = tt.addptr %a_ptrs_47, %a_ptrs_33 : tensor<64x32x!tt.ptr<f16>, #blocked>, tensor<64x32xi32, #blocked> loc(#loc52)
      %b_ptrs_56 = tt.addptr %b_ptrs_45, %b_ptrs_35 : tensor<32x64x!tt.ptr<f16>, #blocked2>, tensor<32x64xi32, #blocked2> loc(#loc54)
      scf.yield %b_ptrs_56, %acc_53, %a_ptrs_55 : tensor<32x64x!tt.ptr<f16>, #blocked2>, tensor<64x64xf32, #linear>, tensor<64x32x!tt.ptr<f16>, #blocked> loc(#loc2)
    } loc(#loc64)
