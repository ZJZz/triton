...
#
# Usage:
#   ./learn_triton/scripts/compile_and_dump.sh <kernel.py> <kernel_name> "<signature>" "<grid>" <out_subdir>
#
# Example (vector add):
#   ./learn_triton/scripts/compile_and_dump.sh learn_triton/kernels/vec_add.py add_kernel "*fp32:16, *fp32:16, *fp32:16, i32, 1024" "1024,1,1" vecadd
#
...
MLIR_ENABLE_DUMP=1 \
MLIR_DUMP_AFTER_PASS=$DUMP_AFTER \
"$PY" "$ROOT/learn_triton/tools/compile_driver.py" \
  "$KERNEL_FILE" \
  "$KERNEL_NAME" \
  "$SIGNATURE" \
  "$ARCH" \
