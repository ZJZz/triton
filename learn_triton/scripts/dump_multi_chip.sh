...
#
# Usage: ./learn_triton/scripts/dump_multi_chip.sh <file.py> <kernel> "<signature>" "<grid>" <out_subdir> [num_stages]
#
...
for ARCH in 75 80 86 90; do
  DUMP=$ROOT/learn_triton/dumps/${OUT}/sm${ARCH}
  ...
  "$PY" "$ROOT/learn_triton/tools/split_pass_dump.py" "$DUMP/mlir-pass-dump.log" "$DUMP/mlir-pass-dump.split" >/dev/null
